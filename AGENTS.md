...
7. **Cloudflare Images**: `siteImage()` falls back to git `/images/` until GitHub secrets `CF_ACCOUNT_ID` + `CF_IMAGES_TOKEN` exist. Runbook: `docs/CLOUDFLARE-IMAGES.md`. `.github/workflows/cloudflare-images-sync.yml` always runs `npm run test:images` plus a dry-run plan; it **skips upload and hash commit** when secrets are empty (job stays green, summary says blocked). After secrets exist, the Action uploads `public/images` and commits `src/lib/cloudflare-account-hash.ts` so production can serve `imagedelivery.net`. Optional: Vercel `NEXT_PUBLIC_CF_IMAGES_BASE_URL` and a DNS-only CNAME for `images.skyecanyonhomesforsale.com`. Do not orange-cloud the Vercel `www` record. Cloudflare MCP auth is desktop-only; this environment cannot create those secrets.
